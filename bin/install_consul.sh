...
# 校验json是否合法，否则提示
if ! docker run --rm -v /etc/consul.d:/etc/consul.d consul:"${CONSUL_VERSION}" validate /etc/consul.d; then
    log "consul 配置文件校验失败，请根据stderr的错误提示修复"
    exit 1
fi

log "删除已存在的consul容器"
if docker ps | awk '{print $NF}' | grep -wq "bk-consul"; then
  log "检测到已存在的bk-consul，删除"
  docker rm -f bk-consul
fi

log "启动consul容器"
docker run --name=bk-consul -d -v /etc/consul.d:/etc/consul.d -v /var/lib/consul:/var/lib/consul --net=host -e CONSUL_ALLOW_PRIVILEGED_PORTS=53 consul:1.7.9 agent -config-dir=/etc/consul.d -config-dir=/etc/consul.d/service -data-dir=/var/lib/consul

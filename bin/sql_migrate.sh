...
if ! docker exec -i mysql mysql --login-path="$MYSQL_LOGIN_PATH" -e 'show processlist' >/dev/null; then
...
for sql in "${SQL[@]}"; do
    ...
    if [[ -f $MIGRATE_DIR/${sql_name} ]]; then
        ...
        if docker exec -i mysql mysql --login-path="$MYSQL_LOGIN_PATH" < "$sql"; then

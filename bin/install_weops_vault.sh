...
if [[ "$INIT" == true ]]; then
    ...
    if docker exec vault sh -c "export VAULT_ADDR=$VAULT_ADDR; vault status -format=json" | grep -q '"initialized":[[:space:]]*true'; then
        ...
        export VAULT_UNSEAL_CODE=$(cat /data/vault.secret|grep 'Unseal Key'|awk '{print $4}')
        if [ -z "$VAULT_UNSEAL_CODE" ]; then
            error '\033[31mError: VAULT_UNSEAL_CODE is empty!\033[0m\n'
        else
            docker exec vault sh -c "export VAULT_ADDR=http://127.0.0.1:8200 && vault operator unseal ${VAULT_UNSEAL_CODE}"
        fi

        export VAULT_ROOT_TOKEN=$(cat /data/vault.secret|grep 'Initial Root Token'|awk '{print $4}')
        if [ -z "$VAULT_ROOT_TOKEN" ]; then
            error '\033[31mError: VAULT_ROOT_TOKEN is empty!\033[0m\n'
        else
            docker exec vault sh -c "export VAULT_ADDR=http://127.0.0.1:8200 && export VAULT_TOKEN=${VAULT_ROOT_TOKEN} && vault secrets enable -path=secret kv"
        fi
    fi
fi
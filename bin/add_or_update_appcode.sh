...
if [[ -n "$(${mysql_cli} --login-path=$LOGIN_PATH -D open_paas -e "select * from esb_app_account where app_code = \"$APP_CODE\"" 2>/dev/null)" ]] ; then
...
if ${mysql_cli} --login-path=$LOGIN_PATH -D open_paas -e \
...
wlist=$(${mysql_cli} --login-path=$LOGIN_PATH -D open_paas -sNe "select wlist from esb_function_controller where func_code = 'user_auth::skip_user_auth'")

if ! echo "$wlist" | grep -qw $APP_CODE; then
    if ${mysql_cli} --login-path=$LOGIN_PATH -D open_paas -e "update esb_function_controller set wlist=concat(wlist, ',$APP_CODE') where func_code = 'user_auth::skip_user_auth'"; then
